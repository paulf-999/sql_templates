...
create_snowflake_account_objs:
	...
	@${SNOWSQL_QUERY} -f eg_bikestore_db/snowflake/account_objects/database/v1_analytics_db_and_schemas.sql --variable PROGRAM=${PROGRAM}
	...
	@${SNOWSQL_QUERY} -f eg_bikestore_db/snowflake/account_objects/role/permissions/grant_permissions/v1_grant_role_permissions.sql --variable PROGRAM=${PROGRAM} --variable ENV=${ENV}

create_snowflake_raw_db_objs:
	$(info [+] Create the snowflake RAW db objects)
	${SNOWSQL_QUERY} -f eg_bikestore_db/snowflake/database_objects/raw_db/table/v1_tbls.sql --variable PROGRAM=${PROGRAM}

populate_snowflake_raw_db_tbls:
	$(info [+] Populate the RAW db tables)
	${SNOWSQL_QUERY} -f eg_bikestore_db/snowflake/load_data.sql --variable PROGRAM=${PROGRAM}
